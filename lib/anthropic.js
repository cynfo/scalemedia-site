...
function getConfig() {
  return {
    // Standard: Haiku 4.5 - raskt nok til å bli ferdig innenfor Vercel Hobby
    // sin 60s-grense (~30s per side). Sett ANTHROPIC_MODEL=claude-sonnet-4-6 for
    // rikere sider (krever Vercel Pro pga. lengre kjøretid).
    model:     process.env.ANTHROPIC_MODEL || 'claude-haiku-4-5',
    maxTokens: parseInt(process.env.MAX_TOKENS || '9000', 10),
    // Pris per million tokens (USD). Standard ~ Haiku 4.5. Juster i .env ved behov.
    inUsdPerMtok:  parseFloat(process.env.PRICE_IN_PER_MTOK  || '1'),
    outUsdPerMtok: parseFloat(process.env.PRICE_OUT_PER_MTOK || '5'),
    // Under 60s slik at vi rekker en vennlig feilmelding før Vercel Hobby
    // hard-stopper funksjonen på 60s.
    timeoutMs: parseInt(process.env.GENERER_TIMEOUT_MS || '55000', 10)
  };
}
