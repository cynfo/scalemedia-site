...
const { streamMessage, logUsage, getConfig, ApiKeyMissingError } = require('./anthropic');
...
// Grovt estimat på ventetid (sekunder) per modell – vises til brukeren.
function estimateSeconds(model) {
  const m = String(model || '').toLowerCase();
  if (m.includes('opus'))   return 210;  // ~3-4 min
  if (m.includes('sonnet')) return 110;  // ~2 min
  return 35;                             // Haiku o.l. ~30-40 s
}
...
async function runStream(res, { action, system, userContent, ip }) {
  ...
  // Fortell klienten hvor lang tid det omtrent tar (avhenger av modellen).
  send({ type: 'meta', estSeconds: estimateSeconds(getConfig().model) });
  ...
  try {
    ...
    if (!html || !/<html|<!doctype html/i.test(html)) {
      ...
      // Bytt {{IMG:...}}-plassholdere med ekte, relevante bilder. Feiler det,
      // brukes HTML-en som den er (modellen har gradient-fallback bak bildene).
      try { html = await resolveImages(html); } catch (e) { console.error('Bilde-resolver feilet:', e.message); }
